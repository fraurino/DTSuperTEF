...
type
  ...
  public
    ...
    function RejeitarPagamento(UniqueId: Integer): TJSONObject;
...
function TDTSuperTEF.RejeitarPagamento(UniqueId: Integer): TJSONObject;
begin
...
function TDTSuperTEF.CriarPagamento(const ClienteChave: string; POSId: Integer; const TransactionType: string;
  InstallmentCount, InstallmentType: Integer; Amount: Currency; const OrderID, Description: string): TJSONObject;
var
  JSON: TJSONObject;
begin
  JSON := TJSONObject.Create;
  try
   ...
   JSON.AddPair('amount', TJSONNumber.Create(StringReplace(FloatToStrF(Amount, ffFixed, 15, 2), ',', '.', [rfReplaceAll])));
