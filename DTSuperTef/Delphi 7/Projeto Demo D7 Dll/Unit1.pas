...
uses
 ...
 procedure Pagamento_Detalha(authorization_sh: WideString;
  ...
  procedure Pagamento_acquirer_banco(Result: PAnsiChar); stdcall; external 'supertef.dll' name 'Pagamento_acquirer_banco';
  procedure Pagamento_acquirer_cnpj(Result: PAnsiChar); stdcall; external 'supertef.dll' name 'Pagamento_acquirer_cnpj';
type
  TForm1 = class(TForm)
...
Pagamento_Detalha(authorization_sh,strtoint(Edit12.text),pResult);
  ...
  pResult := '';
  Pagamento_acquirer_banco(pResult);
  memo7.Lines.Add('acquirer_banco: '+pResult);
  pResult := '';
  Pagamento_acquirer_cnpj(pResult);
  memo7.Lines.Add('acquirer_cnpj: '+pResult);
   end;
